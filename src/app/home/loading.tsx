    ...
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-4 py-8 md:px-6">
      <header className="flex flex-col gap-5 border-b border-border pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl space-y-3">
          ...
          <div className={`h-10 w-full max-w-lg rounded ${SHIMMER}`} />
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={index}
            className="rounded-md border border-border bg-secondary/15 p-4"
          >
            <div className="flex items-center justify-between gap-3">
              <div className={`h-4 w-20 rounded ${SHIMMER}`} />
              <div className={`h-4 w-4 rounded ${SHIMMER}`} />
            </div>
            <div className={`mt-3 h-8 w-20 rounded ${SHIMMER}`} />
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              ...
              >
                ...
                <div className={`mt-1 h-3 w-20 rounded ${SHIMMER}`} />
              </div>
            ))}
          </div>
        </div>
        <div className="grid grid-cols-[6rem_1fr] gap-4 rounded-md border border-border bg-secondary/15 p-4">
