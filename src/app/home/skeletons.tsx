// Loading placeholders for the home page sections. Each one mirrors the
        ...
        <ul className="flex gap-4 pb-2">
          {Array.from({ length: count }).map((_, i) => (
            <li key={i} className="w-36 shrink-0 sm:w-40">
              <div className={`aspect-square rounded-cover ${SHIMMER}`} />
...
function ChartRowSkeleton() {
  return (
    <li className="grid min-h-20 grid-cols-[2rem_3.5rem_1fr] items-center gap-4 rounded-xl bg-card px-3 py-2.5">
      <div className={`h-3 w-5 rounded-xl ${SHIMMER}`} />
      <div className={`h-14 w-14 rounded-cover ${SHIMMER}`} />
